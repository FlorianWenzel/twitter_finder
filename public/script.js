...
socket.on('prepareing_download', (max, current) => {
  if (current === 0)
    Swal.fire({
      ...
      type: '',
      html: 'depending on your query this might take a while<br><br>' +
        `<div class="progress">
...
socket.on('getKeys', (keys) => {
  ...
  Swal.fire({
      ...
      type: '',
      html: 'Current tags:<br>' + badges,
      showCloseButton: true,
      showCancelButton: true,
      focusConfirm: false,
      confirmButtonText: '<i class="fa fa-plus"></i>',
      confirmButtonAriaLabel: 'Add tag',
      cancelButtonText: '<i class="fa fa-minus"></i>',
...
socket.on('getBannedKeys', (banned_keys) => {
  ...
  Swal.fire({
      ...
      buttonsStyling: false,
      type: '',
      html: 'Current banned tags:<br>' + badges,
      showCloseButton: true,
      showCancelButton: true,
      focusConfirm: false,
      confirmButtonText: '<i class="fa fa-plus"></i>',
...
socket.on('tweet', (tweet) => {
  $('#tweets').prepend(`
    <div class="card bg-primary text-white mb-3">
      ...
      <div class="card-body ">
        <a href="'https://twitter.com/'${tweet.user.screen_name}" target="_blank" class="">
        ...
        <p class="card-text">
          <small class="">${(new Date(parseInt(tweet.timestamp_ms))).toLocaleString().substring(0, 20)}
          </small>
        </p>
      </div>
    </div>
...
socket.on('power', (running, timestamp) => {
  if (running) {
    $('#power').removeClass('btn-success')
    $('#power').addClass('btn-danger')
    $('#power').html('<i class="fas fa-power-off"></i> stop')
  } else {
    $('#power').removeClass('btn-danger')
    $('#power').addClass('btn-success')
    $('#power').html('<i class="fas fa-power-off"></i> start')
  }
})
